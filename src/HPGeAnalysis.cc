...
void HPGeAnalysis::CalcEfficiencies() {
    ...
    for (int i=0; i<fnlines; ++i) {
        ...
        double width_bck = fwith_bkg[i];
        double BR_ratio = fNuDatBR[i]/fG4BR[i];
        ...
        fefficiency.push_back(efficiency*BR_ratio);
        fefficiency_err.push_back(efficiency_err*BR_ratio);
